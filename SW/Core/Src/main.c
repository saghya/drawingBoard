...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    while (1)
    {
        ...
        a = HAL_GPIO_ReadPin(BTN1_GPIO_Port, BTN1_Pin);
        b = HAL_GPIO_ReadPin(BTN2_GPIO_Port, BTN2_Pin);
        c = HAL_GPIO_ReadPin(BTN3_GPIO_Port, BTN3_Pin);

        if (!a | !b | !c) {
            TIM8->CCR4 = 0;
        } else {
            TIM8->CCR4 = ~HAL_ADC_GetValue(&hadc1) << 4;
        }
    /* USER CODE END WHILE */
